...
class C(BaseConstants):
    ...
    # 2回目のタスクの確率（%）
    RISKY_PROBABILITIES_SECOND = [70, 60, 75, 80, 50, 90, 20, 65, 85, 30] 

    #２回目のタスクの報酬設定
    SAFE_REWARDS_SECOND = [
        cu(150), cu(700), cu(450), cu(650), cu(200), 
        cu(700), cu(200), cu(300), cu(750), cu(250)
    ]
    RISKY_SUCCESS_REWARDS_SECOND = [
        cu(750), cu(900), cu(800), cu(700), cu(600),
        cu(990), cu(850), cu(700), cu(940), cu(950)
    ]
...
def generate_unique_passcode(existing_codes):
    # 6桁の数字
    while True:
...
def creating_session(subsession: Subsession):
    ...
    for player in subsession.get_players():
        code = generate_unique_passcode(used_codes)
        player.passcode = code
        used_codes.add(code)
...
class Player(BasePlayer):
    ...
    # リスク選好（1問）
    risk_preference = models.IntegerField(choices=[1,2,3,4,5,6,7], widget=widgets.RadioSelect)
    # 年齢
    ...
    passcode = models.StringField()
    entered_passcode = models.StringField()
...
class Exit(Page):
    form_model = 'player'
    form_fields = ['entered_passcode']

    @staticmethod
    def vars_for_template(player: Player):
        return dict(passcode=player.passcode)

    @staticmethod
    def error_message(player: Player, values):
        if values['entered_passcode'] != player.passcode:
            return '入力されたパスコードが正しくありません。'
